...
SELECT * FROM Товар
UPDATE ПроизводителиТовара SET Производитель = '' WHERE Код = 0
UPDATE Поставки SET ДатаПоставки = GETDATE(), Товар = 0, Цена = 0, Количество = 0, Стоимость = 0 WHERE Код = 0;
UPDATE Продажи SET Товар = 0, ЦенаПродажи = 0, Количество = 0 ,Стоимость = 0 WHERE Код = 0
  SELECT Код FROM Товар WHERE Код = (SELECT MAX(Код) FROM Товар)
SELECT MAX(Код) FROM  Продажи
  SELECT * FROM Поставки WHERE ДатаПоставки = ''
SELECT * FROM Поставки